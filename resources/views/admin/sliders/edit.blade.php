...
@section('content')
    <section class="section">
        <form method="POST" action="{{route('admin.sliders.update',$slider->id)}}" class="ajaxform"  >
            @csrf
            @method('PUT')
            <div class="row">
                    ...
                    <div class="card">
                        <div class="card-body  ">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" value="{{$slider->button_text}}"   name="button_text"  class="form-control"   >
                            </div>
                            <div class="form-group">
                                <label for="button_link" class="required">{{ __('Button Link') }} </label>
                                <input type="text" value="{{$slider->button_link}}"   name="button_link"  class="form-control"   >
                            </div>
                            ...
                            {{ mediasection([
                                       ...
                                       'preview_class' => 'banner',
                                        'preview' => $slider->banner ?? null,
                                        'value' => $slider->banner ?? null
                           ]) }}
                            ...
                            {{ mediasection([
                                       ...
                                       'preview_class' => 'child_image',
                                        'preview' => $slider->child_image ?? null,
                                        'value' => $slider->slide_image ?? null
                           ]) }}
                        ...
                        <div class="card-body  ">
                            <div class="form-group">
                                ...
                                <select name="status" class="form-control">
                                    <option @selected($slider->status == 1) value="1">{{ __('Active') }}</option>
                                    <option @selected($slider->status == 0)  value="0">{{ __('Inactive') }}</option>
                                </select>
                            </div>
                            <div class="form-group">
